...
driver_name="RoadConditionViolationDetectionDriver"
app_name="Analytics_RoadConditionViolationDetection"
factory_class_name="com.epam.tcodata.analytics.road.condition.violation.detection.factory.impl.RoadConditionViolationFactory"
batchIntervalSeconds=120

/usr/hdp/${spark_ver}/spark2/bin/spark-submit \
--class com.epam.tcodata.analytics.road.condition.violation.detection.${driver_name} \
--name ${app_name} \
--master yarn \
--deploy-mode cluster \
`# SPARK CONFIGURATION` \
...
${jar_path}/road-condition-violation-detection-${jar_ver_suffix}.jar \
--appName ${app_name} \
