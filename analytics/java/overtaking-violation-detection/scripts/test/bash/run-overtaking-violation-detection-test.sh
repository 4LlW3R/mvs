...
driver_name="OvertakingViolationDetectionDriver"
app_name="Analytics_OvertakingViolationDetection"
factory_class_name="com.epam.tcodata.analytics.overtaking.violation.detection.factory.impl.OvertakingViolationFactory"
batchIntervalSeconds=210

/usr/hdp/${spark_ver}/spark2/bin/spark-submit \
--class com.epam.tcodata.analytics.overtaking.violation.detection.${driver_name} \
--name ${app_name} \
--master yarn \
--deploy-mode cluster \
`# SPARK CONFIGURATION` \
...
${jar_path}/overtaking-violation-detection-${jar_ver_suffix}.jar \
