...
import com.epam.tcodata.road.condition.violation.stream.datalake.exception.ArgsException;
import com.epam.tcodata.road.condition.violation.stream.datalake.factory.IRoadConditionFactory;
...
public class RoadConditionViolationStreamDataLakeDriver {

    private static final Logger LOGGER = LoggerFactory.getLogger(RoadConditionViolationStreamDataLakeDriver.class);
    ...
    private static final Long EVENT_TYPE_ID = 400L;


    /**
     * Main driver class of RoadConditionViolationStreamDataLake job.
     *
    ...
    public static void main(String[] args) throws Exception {
        ...
        IRoadConditionFactory factory = FactoryUtil.loadFactory(IRoadConditionFactory.class, factoryClassName);

        JavaStreamingContext jsc = handleJavaStreamingContext(factory, appName, batchIntervalSeconds);
        runPump(jsc);
    }

    private static JavaStreamingContext handleJavaStreamingContext(IRoadConditionFactory factory,
                                                                   String appName,
        ...
        JavaDStream<EventData> eventDataDStream = eventHub
                .receiveStream(jsc, ApplicationType.ROAD_CONDITION_VIOLATION_STREAM_DATALAKE.getConsumerGroup());
    ...
    private static void handle(IRoadConditionFactory factory,
                               ISecretStorage secretStorage,
                               SparkSession sparkSession,
                               JavaRDD<EventData> eventDataRDD) throws Exception {

        JavaRDD<RoadConditionViolationAvro> avroViolationRDD = eventDataRDD
                .map(EventData::getBytes)
                .map(bytes -> AvroSerDeUtil.deserialize(RoadConditionViolationAvro.class, bytes))
                .persist(StorageLevel.MEMORY_AND_DISK_SER());

        IRoadConditionConverter converter = factory.createRoadConditionConverter();
        JavaRDD<PreparedRoadConditionViolation> violationRDD = avroViolationRDD
                .map(converter::convertToRoadConditionViolation)
                .coalesce(1)
                .persist(StorageLevel.MEMORY_AND_DISK_SER());
        Timestamp persistedDateUtc = Timestamp.from(Instant.now());
        JavaRDD<RawDetectedEvent> dataLakeEventRDD = avroViolationRDD
                .map(avro -> converter.convertToDataLakeEvent(avro, EVENT_TYPE_ID, persistedDateUtc))
                .coalesce(1)
                .persist(StorageLevel.MEMORY_AND_DISK_SER());

        avroViolationRDD.unpersist();
        if (!violationRDD.isEmpty()) {
            IHiveRepository confirmedOvertakingRepository = factory.createPreparedHive(sparkSession).repository(PreparedAreaEntityType.ROAD_CONDITION_VIOLATION);
            JavaRDD<Row> violationRowRDD = HiveCommon.entityRddToRowRdd(violationRDD);
            Dataset<Row> violationRowDataset = HiveCommon.rowRddToRowDataset(violationRowRDD, PreparedRoadConditionViolation.class, sparkSession);
